...
apiKeySchema.statics.findActiveByUserAndProvider = function(userId, provider = 'openai') {
  return this.findOne({ userId, provider, isActive: true }).populate('userId', 'name email');
};

// Static method to deactivate all keys for a user and provider
apiKeySchema.statics.deactivateForUserAndProvider = function(userId, provider = 'openai') {
